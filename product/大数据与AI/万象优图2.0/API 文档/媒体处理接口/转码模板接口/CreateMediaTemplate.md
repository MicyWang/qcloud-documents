...
| Width              | Request.Video | 宽                 | String | 否       | 视频原始宽度                   | <li> 值范围：[128, 4096]<br/><li> 单位：px<br/><li> 若只设置Width时, 按照视频原始比例计算Height |
| Height             | Request.Video | 高                 | String | 否       | 视频原始高度                   | <li> 值范围：[128, 4096]<br/><li> 单位：px<br/><li> 若只设置Height时, 按照视频原始比例计算Width |
...
| Crf                | Request.Video | 码率-质量控制因子  | String | 否       | 无                             | <li> 值范围：(0, 51]<br/><li> 如果设置了Crf, 则Bitrate的设置失效<br/><li>默认为不设置 Crf |
| Gop                | Request.Video | 关键帧间最大帧数   | String | 否       | 无                             | <li> 值范围：[0, 100000]<br/><li>默认不设置 Gop<br/><li> Codec为VP8不支持此参数 |
| Preset             | Request.Video | 视频算法器预置     | String | 否       | medium                         | <li> 仅H.264支持该参数<br/><li> 取值veryfast、fast、medium、slow、slower<br/><li> Codec为VP8不支持此参数 |
| Bufsize            | Request.Video | 缓冲区大小         | String | 否       | 无                             | <li> 值范围：[1000, 128000]<br/><li> 单位：Kb<br/><li> Codec为VP8不支持此参数<br/><li>默认值为0，表示不使用缓冲区 |
| Maxrate            | Request.Video | 视频码率峰值       | String | 否       | 无                             | <li> 值范围：[10, 50000]<br/><li> 单位：Kbps<br/><li> Codec为VP8不支持此参数 <br/><li>默认值为0，表示不使用此参数 |
| HlsTsTime          | Request.Video | hls分片时间        | String | 否       | 5                              | <li> (0 视频时长] <br/> <li> 单位为秒<br/><li> Codec为VP8不支持此参数 |
...
| 节点名称（关键字） | 父节点        | 描述         | 类型   | 必选 | 默认值                               | 限制                                                         |
| ------------------ | ------------- | ------------ | ------ | ---- | ------------------------------------ | ------------------------------------------------------------ |
| Codec              | Request.Audio | 编解码格式   | String | 否   | aac，当format为WebM时，为Vorbis      | 取值 aac、mp3、flac、amr、Vorbis                             |
...
| 节点名称（关键字） | 父节点                         | 描述            | 类型   | 必选 | 默认值 | 限制                                                         |
| ------------------ | ------------------------------ | --------------- | ------ | ---- | ------ | ------------------------------------------------------------ |
| IsHlsEncrypt       | Request.TransConfig.HlsEncrypt | 是否开启HLS加密 | String | 否   | false  | 1. true/false<br/>2. 当Video.Codec为H264或H265并且Container.Format为hls时支持加密 |
